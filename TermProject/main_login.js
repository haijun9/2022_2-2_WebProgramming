...
$("#cancel").click(modalClose); // 로그인 모달 창 속 취소 클릭 시
...
$("#main").click(function() {location.href='./Main.html';}) // 배너 클릭 시 메인 페이지로
$("#signup").click(function() {location.href='./SignUp.html';}) // 회원가입 클릭 시 회원가입 페이지로

$("#ok").click(function(){    // 로그인 시도 시
  if($("#ID").val()=="" ||  $("#pwd").val()=="") alert("아이디 또는 패스워드가 모두 입력되었는지 확인해주세요.");
  // validation 실패 시
  else {  // validation 통과 시
      $.ajax({
          url: "login.php",      // login.php로 수행
          type: "post",
          data: {id: $("#ID").val(), pwd: $("#pwd").val()}, // ID와 pwd 전달
          dataType: "json",
